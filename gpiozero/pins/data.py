...
class PiBoardInfo(namedtuple('PiBoardInfo', (
    ...
    def find_gpio(self, spec):
        ...
        * 'GPIOn' or 'BCMn' where n is the GPIO number
        * 'WIPIn' or 'WPIn' where n is the `wiringPi`_ pin number
        * 'PHYSn' where n is the physical pin number on header P1
        * 'h:n' where h is the header name and n is the physical pin number
          (for example P1:5 is physical pin 5 on header P1)
        ...
        if spec.startswith('GPIO') and spec[4:].isdigit():
            ...
            if pin.isdigit():
                try:
                    ...
                    raise PinInvalidPin(
                        'there is no %s header on the Pi' % header)
                try:
                ...
                if function.startswith('GPIO') and function[4:].isdigit():
                    return int(function[4:])
                else:
                    raise PinInvalidPin('%s is not a GPIO pin' % spec)
        elif spec.startswith('PHYS') and spec[4:].isdigit():
            main_head = 'P1' if 'P1' in self.headers else 'J8'
            return self.find_gpio('%s:%s' % (main_head, spec[4:]))
        elif spec.startswith('BCM') and spec[3:].isdigit():
            return self.find_gpio('GPIO%s' % spec[3:])
        elif spec.startswith('WIPI') and spec[4:].isdigit():
            return self.find_gpio('WPI%s' % spec[4:])
